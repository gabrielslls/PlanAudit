...
TEST_CASES = [
    ...
    {
        "name": "代码生成",
        "messages": [{"role": "user", "content": "用 Python 写一个斐波那契数列函数"}],
        "max_tokens": 200,
    },
    {
        "name": "文本总结",
        "messages": [{"role": "user", "content": "请用一句话总结什么是机器学习"}],
        "max_tokens": 100,
    },
    {
        "name": "角色扮演",
    ...
    },
    {
        ...
        "messages": [
            {
                "role": "user",
                "content": "请详细解释 Python 中的装饰器模式，包括原理、用法和示例代码",
            }
        ],
        "max_tokens": 300,
    },
    {
        ...
        "messages": [
            ...
            {"role": "assistant", "content": "Git 是一个分布式版本控制系统。"},
    ...
    },
    {
        ...
        "messages": [
            {
                "role": "user",
                "content": "以 JSON 格式输出一个包含 name、age、city 字段的人员信息示例",
            }
        ],
        "max_tokens": 100,
    },
    {
        ...
        "messages": [
            {
                "role": "user",
                "content": "将以下英文翻译成中文: Artificial Intelligence is transforming every industry.",
            }
        ],
        "max_tokens": 100,
    },
    {
        ...
        "messages": [
            {
                "role": "user",
                "content": "如果 A 比 B 大，B 比 C 大，那么 A 和 C 谁大？为什么？",
            }
        ],
        "max_tokens": 100,
    },
...
def call_api(
    api_base: str, api_key: str, model: str, messages: list, max_tokens: int = 100
) -> dict:
...
def run_tests(api_base: str, api_key: str, model: str, test_count: int):
    ...
    for i, test_case in enumerate(TEST_CASES[:test_count], 1):
        ...
        if "error" in response:
            print(
                f"❌ 失败 ({response.get('status', 'N/A')}): {response.get('message', '')[:50]}"
            )
            error_count += 1
            results.append(
                {
            ...
            print(
                f"✅ prompt={prompt_tokens}, completion={completion_tokens}, total={tokens}, {int(elapsed * 1000)}ms"
            )
    ...
    if success_count > 0:
        avg_time = (
            sum(r["elapsed_ms"] for r in results if r["status"] == "success")
            // success_count
        )
    ...
    output = {
        ...
        "config": {"api_base": api_base, "model": model, "test_count": test_count},
        "summary": {
...
示例:
  ...
  python test-non-streaming.py deepseek-chat --count 5
    ...
    parser.add_argument(
        ...
        help="API 地址（默认: http://127.0.0.1:9000）",
    )
